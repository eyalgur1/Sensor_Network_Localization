function out=alg_AMCC(Network,x0,u0,STOP,varargin)
%% INFORMATION

% DESCRIPTION: implements AMCC (AMU with colored clusters), with/without warm start.
...
% generating colored clusters
clusters_strct=MakeClusters(Network,{});
clusters=clusters_strct.clusters;
q=length(clusters); % number of clusters
...
if ~isempty(varargin) % with warm start
    warm_start=varargin;
    if ~isempty(strfind(warm_start{1},'SF')) % SF warm start
        outWarm=alg_SF(Network,x0,warm_start{2},u0);
    elseif ~isempty(strfind(warm_start{1},'AG')) % AG warm start
        outWarm=alg_AG(Network,x0,u0,warm_start{2},warm_start{3});
    end
    
    fun_v(1:warm_start{2})=outWarm.function_values(2:end); % update function values without the starting value F(x0,u0)
    norm2_diff(1:warm_start{2})=outWarm.norm2_diff(2:end);
    RMSE_v(1:warm_start{2})=outWarm.RMSE(2:end); % update RMSE values without the starting value RMSE(x0)
    ...
    x1=reshape(outWarm.location_estimation(:,1:N),n,N); % new x0 as starting point obtained by the warm start
    [u1,~,~]=u_update(x1,u0,v_x_coef,v_const,Ne,n,K,[]); % new u0 as starting point obtained by the warm start
    fun_v(warm_start{2})=F(x1,u1); % update function value of the new starting point
    norm2_diff(warm_start{2})=sum(sum((x1(:,1:N)-Xreal(:,1:N)).^2));
    RMSE_v(warm_start{2})=RMSE(x1); % update RMSE value of the new starting point
    x=x1;
    u=u1;
    xk=x;
    uk=u;
    iter=warm_start{2}; % update the total iteration counter
else % no warm start
    warm_start=false;
    x=x0;
    u=u0;
...
while all(stopping(x,u,xk,uk,iter))==0 || first_iteration
    ...
    starta=tic; % upper bound on total run time
    x=xk;
    u=uk;
    ...
    for i=1:q % update the x block
        ...
        endi(i)=toc(starti)/size(S_i.Ptilde,1); % run time of the cluster, (approximately) parallelized among the cluster's sensors
    end

    [uk,u_time,~]=u_update(xk,uk,v_x_coef,v_const,Ne,n,K,edges); % update u
    u_max_time=max(u_time(1:N)); % maximal u computation time for each processor (sensor)
    ...
    tp(iter)=sum(endi)+u_max_time; % (approximated) parallel run time
    
    % print values every 50 iterations
    if ~mod(iter,100) || iter==1
        fprintf('AMCC:    Iter=%2d    RMSE=%10.10f    F(k)-F(k+1)=%10.10f\n',iter,RMSE_v(iter),F(x,u)-F(xk,uk))
    end
end
