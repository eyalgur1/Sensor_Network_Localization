function out=alg_AMFD(Network,x0,u0,STOP,varargin)
%% INFORMATION

% DESCRIPTION: implements AMFD (AMU with N random clusters), with/without warm start.
...
clusters_strct=MakeClusters(Network,{N,'r'});
...
while all(stopping(x,u,xk,uk,iter))==0 || first_iteration
    ...
    [uk,u_time,~]=u_update(xk,uk,v_x_coef,v_const,Ne,n,K,{}); % update u
    ...
    if ~mod(iter,50) || iter==1
        fprintf('AMFD:    Iter=%2d    RMSE=%10.10f    F(k)-F(k+1)=%10.10f\n',iter,RMSE_v(iter),F(x,u)-F(xk,uk))
    end
end
