function Network=addANchors(net,m_to_add,sigma)
%% INFORMATION

% DESCRIPTION: adds or removes anchors from an existing network, while
%              keeping the number of non-anchors unchanged.

% INPUTS:
% * net - network structure
% * m_to_add - number of anchora to add/remove (non-zero integer)
...
S=(net.Matrices.X_real)';
n=size(S,2);
if m_to_add>0
    S=[S;rand(m_to_add,n)-0.5];
elseif m_to_add<0
    S=(net.Matrices.X_real(:,1:end+m_to_add))';
end
m=net.anchors+m_to_add;
K=size(S,1);
R=net.GI.R;
