...
% 6. SF -        distributed and parallel convex relaxation from the paper
%                https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=7153574&tag=1
...
anchors=[6]; % total number of anchors in the truncated network (must not exceed 10)
...
netsize=[30];%,250,490]; % total number of non-anchors in the truncated network (must not exceed 490)
Ye=1; % if Ye=1 then the networks are generated accoridng to Ye 2008, otherwise according to our method.
radius_ticks=10;
radius=[0.235:(0.55-0.235)/(radius_ticks-1):0.55;... % each row corrensponds to an entry in netsize. A network is generated for each pais of (netsize(i),radius(i,:))
    0.115:(0.215-0.115)/(radius_ticks-1):0.215;...
    0.085:(0.15-0.085)/(radius_ticks-1):0.15];
radius=0.55;
methods={'EMLD','EML'}; % setting the required methods
nr=5; % number of realizations for each network
AMFC_iter=1000; % maximum number of AMFC iterations
SF_iter=1000; % maximum number of AMFC iterations
EMLD_iter=1; % maximum number of EMLD iterations
EMLD_acc=10^-5; % the accuracy of the optimization problem in EMLD
%--------------------------%
...
for i=1:length(netsize)
    N=netsize(i);
    for an=1:length(anchors)
        m=anchors(an);
        strm=['m',num2str(m)];
        for r=1:length(radius(i,:))
            R=radius(i,r);
            strR=['R',strrep(num2str(R),'.','')]; % radius of the network
            data.(['net',num2str(N)]).(strm).(strR).funv_real=zeros(nr,1); % initailize the average function value in the real location with zeros
            for j=1:length(methods) % setting the required metrics (function values and PRMSE) as 'funv' and 'norm2_diff'
                data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv=zeros(nr,1); % initialize the function values of each method and realization with zeros
                data.(['net',num2str(N)]).(strm).(strR).(methods{j}).norm2_diff=zeros(nr,1); % initialize the MSE of each method and realization with zeros
                data.(['net',num2str(N)]).(strm).(strR).(methods{j}).sum_norm2_diff=0; % initailize the MSE with zeros
            end
        end
    end
end
        ...
        for r=1:length(radius(i,:))
            ...
            for l=1:nr
                ...
                for j=1:length(methods)
                    ...
                    elseif contains('SF',methods{j})
                        alg_output=methods_alg(net,x0,SF_iter); % run SF
                    elseif contains('EMLD',methods{j})
                        alg_output=methods_alg(net,EMLD_iter,EMLD_acc); % run EMLD
                    else
                    ...
                    data.(['net',num2str(N)]).(strm).(strR).(methods{j}).X{l}=X_output; % save the algorithm's output location
                    
                    data.(['net',num2str(N)]).(strm).(strR).(methods{j}).norm2_diff(l)=sum(sum((X_output(:,1:N)-Xreal(:,1:N)).^2)); % norm2_diff for MSE
                    data.(['net',num2str(N)]).(strm).(strR).(methods{j}).sum_norm2_diff=data.(['net',num2str(N)]).(strm).(strR).(methods{j}).sum_norm2_diff+data.(['net',num2str(N)]).(strm).(strR).(methods{j}).norm2_diff(l); % sum_norm2_diff is sum over the realizations
                    for e=1:size(edges,1) % function value
                        edge=edges(e,:);
                        data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv(l)=data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv(l)+(norm(X_output(:,edge(2))-X_output(:,edge(3)))-edge(4))^2;
                    end
                end
                
                for e=1:size(edges,1) % calculate the real function value of the realization
                    edge=edges(e,:);
                    data.(['net',num2str(N)]).(strm).(strR).funv_real(l)=data.(['net',num2str(N)]).(strm).(strR).funv_real(l)+(norm(Xreal(:,edge(2))-Xreal(:,edge(3)))-edge(4))^2;
                end
                
                data.(['net',num2str(N)]).(strm).(strR).nets{l}=net; % save the network structure of the realization
                
            end
            
            for j=1:length(methods) % calculate PRMSE, average function value and function value std over the realizations
                sum_norm2_diff=data.(['net',num2str(N)]).(strm).(strR).(methods{j}).sum_norm2_diff;
                data.(['net',num2str(N)]).(strm).(strR).(methods{j}).PRMSE=sqrt(sum_norm2_diff/nr)/N; % PRMSE of each method for a specfiic network
                data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv_avg=sum(data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv)/nr; % average function value of each method for a specfiic network
                data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv_std=std(data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv); % std of the function values of each method for a specfiic network
            end
            
            % calculate the average and std of the real function value over the realizations
            data.(['net',num2str(N)]).(strm).(strR).funv_real_avg=sum(data.(['net',num2str(N)]).(strm).(strR).funv_real)/nr; % average function value in the real location for a specfiic network
            data.(['net',num2str(N)]).(strm).(strR).funv_real_std=std(data.(['net',num2str(N)]).(strm).(strR).funv_real); % std of the function value in the real location for a specfiic network
        end
    end
end
    ...
    for an=1:length(anchors)
        ...
        for j=1:length(methods) % each plot contains a grpah for each method
            for r=1:length(radius(i,:)) % R values are the x axis
                ...
                PRMSEplot(r)=data.(['net',num2str(N)]).(strm).(strR).(methods{j}).PRMSE; % values to plot for each method
            end
        ...
        hold off
        if an<length(anchors) % do not open a new figure after the last graph
            figure()
        end
    end
end
...
for i=1:length(netsize) % plot a figure for each network size
    ...
    for an=1:length(anchors)
        ...
        for r=1:length(radius(i,:)) % R values are compose the x axis
            R=radius(i,r);
            strR=['R',strrep(num2str(R),'.','')];
            funvrealplot(r)=data.(['net',num2str(N)]).(strm).(strR).funv_real_avg; % avg of function value for real location
        end
        ...
        for j=1:length(methods) % each plot contains a grpah for each method
            for r=1:length(radius(i,:)) % R values are compose the x axis
                R=radius(i,r);
                strR=['R',strrep(num2str(R),'.','')];
                funvplot(r)=data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv_avg; % values to plot for each method
            end
        ...
        for j=1:length(methods) % plot std
            for r=1:length(radius(i,:)) % R values are compose the x axis
                R=radius(i,r);
                strR=['R',strrep(num2str(R),'.','')];
                max_vec(r)=max(data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv);
                min_vec(r)=min(data.(['net',num2str(N)]).(strm).(strR).(methods{j}).funv);
            end
