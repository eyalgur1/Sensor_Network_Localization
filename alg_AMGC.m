function out=alg_AMGC(Network,x0,u0,STOP,q,varargin)
%% INFORMATION

% DESCRIPTION: implements AMGC (AMU with geographical clusters), with/without warm start.
...
%               function
% * q - number of required geographical clusters (q<N).
...
clusters_strct=MakeClusters(Network,{q,'g'});
...
while all(stopping(x,u,xk,uk,iter))==0 || first_iteration
    ...
    if ~mod(iter,50) || iter==1
        fprintf('AMGC with %2d clusters:    Iter=%2d    RMSE=%10.10f    F(k)-F(k+1)=%10.10f\n',q,iter,RMSE_v(iter),F(x,u)-F(xk,uk))
    end
